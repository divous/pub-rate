...
export function RatingForm({ visitId, existingRating }: RatingFormProps) {
    ...
    const sliders = [
        { id: 'food', label: 'Food Quality', min: 1, max: 10 },
        { id: 'drink', label: 'Drink Quality', min: 1, max: 10 },
        { id: 'service', label: 'Service', min: 1, max: 10 },
        { id: 'atmosphere', label: 'Atmosphere', min: 1, max: 10 },
        { id: 'value', label: 'Value for Money', min: 1, max: 10 },
    ]

    return (
        <div className="bg-zinc-900/50 rounded-3xl p-6 border border-zinc-800">
            <h3 className="text-lg font-semibold text-zinc-200 mb-6">Your Rating</h3>
            <form action={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2 pt-2">
                    <label htmlFor="note" className="text-zinc-400 text-sm font-medium">Extra Notes</label>
                    <textarea
                        name="note"
                        id="note"
                        rows={3}
